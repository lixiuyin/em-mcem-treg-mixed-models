...
The recorded simulations and empirical runs support three observations: increasing EM objectives, an MCEM sample-size/accuracy/cost trade-off, and estimates approaching EM with sufficient sampling. Comparisons against `optim`/`rlm` and `lme4` provide numerical cross-checks. Use the reported differences and estimands above rather than assuming digit-for-digit agreement across methods; these checks do not prove correctness for every dataset.

**Simulation and empirical library cross-checks.** Simulation comparisons are in §6.1.1 (`optim`/`hett::tlm`/`MASS::rlm`), §6.2.1 (`lme4`, ML and REML) and §6.3.1 (`lme4`/`nlme`, ML and REML), with the corresponding `driver_library_compare.R` scripts. Empirical checks are in §7.1 and §7.2. Match objective functions before comparing: the t-regression MAP $`n+2`$ divisor and ML/REML variance estimators target different quantities. Finite-sample MCEM also has Monte Carlo error, so blanket machine-precision agreement is not claimed.
...
├── README.en.md                    # English derivations, applications and results
